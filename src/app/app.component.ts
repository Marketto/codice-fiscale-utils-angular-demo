...
export class AppComponent implements OnInit {
  public cfFormGroup = new FormGroup(
    {
      ...
      birthDate: new FormControl("", [
        Validators.required,
        CfValidators.birthDate.placeMismatch(
          () => this.cfFormGroup.get("place").value
        )
      ]),
  ...
  private getScopedBelfiore(): BelfioreConnector {
    if (
      this.cfFormGroup.controls.area &&
      this.cfFormGroup.controls.area.value
    ) {
  ...
  public get cfMismatchErrors() {
    return this.getFilteredErrorList(([errorId]) =>
      /cf[a-zA-Z]+Mismatch/.test(errorId)
    );
  }

  public get dateMismatchErrors() {
    return this.getFilteredErrorList(([errorId]) =>
      /[bB]irth[a-zA-Z]*Date[a-zA-Z]*Mismatch/.test(errorId)
    );
  }

  public get placeMismatchErrors() {
    return this.getFilteredErrorList(([errorId]) =>
      /[bB]irth[a-zA-Z]*Place[a-zA-Z]*Mismatch/.test(errorId)
    );
  }
  ...
  ngOnInit() {
    this.filteredAreas = this.cfFormGroup.controls.area.valueChanges.pipe(
      startWith(""),
      map(value => {
        ...
        if (value === CfValidators.constant.COUNTRIES) {
          ...
          return this.areas.filter(area =>
            matcher.test(this.displayArea(area))
          );
        }
        return [];
      })
    ...
    this.filteredPlaces = this.cfFormGroup.controls.place.valueChanges.pipe(
      startWith(""),
      map((value: string | BelfiorePlace) => {
        if (value) {
          if (
            !this.cfFormGroup.controls.area ||
            this.cfFormGroup.controls.area.invalid
          ) {
            return [];
          }
          if (typeof value === "string") {
            const matcher = new RegExp(`^${value}`, "i");
            return (this.getScopedBelfiore().searchByName(value) || []).sort(
              (a, b) =>
                (matcher.test(a.name) ? 0 : 1) -
                  (matcher.test(b.name) ? 0 : 1) ||
